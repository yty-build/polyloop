...
1. Explicit human-owner instructions, including the committed Owner Test Directive and Owner Capital Authorization.
2. `PROJECT_CHARTER.md` technical-integrity rules and the Manager-owned `CAMPAIGN.md` objective, resources, and stop conditions.
3. `CURRENT_EXPERIMENT.md` assignment, exact Experiment Test, and evidence references.
4. `FUNCTIONALITY_LOG.md` verified reusable bot functionality and recorded technical limitations.
5. Your role contract.
6. `LEADERBOARD.md`, `LESSONS.md`, and prior experiment evidence.
...
- Re-read this contract, your role file, `CURRENT_EXPERIMENT.md`, and `FUNCTIONALITY_LOG.md` at every assignment.
...
- Treat the committed Owner Test Directive as exact. A model may report ambiguity or technical impossibility, but may not reinterpret, weaken, strengthen, supplement, or substitute the owner's requested test. A changed owner instruction requires a new committed directive.
...
- Bot Builder acts only after Validator `pass`. It reuses compatible `verified` entries in `FUNCTIONALITY_LOG.md`; it may rebuild one only after a concrete technical limitation and evidence are recorded and Reality approves the smallest necessary change.
- The human owner alone controls whether money is used and its account, market, allocation, sizing, timing, duration or count, and stop conditions. Models may report risks or recommendations, but those statements grant no authority.
- Reality may use money only after paper matches and an exact Owner Capital Authorization is recorded and committed. Never infer authorization from a campaign, prior experiment, prior approval, wallet balance, model suggestion, or default. Missing scope means no authority.
- The bot enforces the owner's exact financial and test instructions. It may also enforce technical-integrity behavior such as artifact identity, duplicate prevention, stale-input rejection, audit logging, and an owner-accessible stop command, but it may not add model-chosen strategy, allocation, sizing, market, duration, or loss rules.
- Stop when the Owner Capital Authorization expires, is exhausted, is revoked, or any owner-defined or technical-integrity condition fails. Never extend, redirect, reuse, or scale an authorization automatically.
...
1. Exact Owner Test Directive before Council starts.
2. Council Result.
3. Approved, frozen Experiment Test before Builder starts.
4. Builder Result, including the immutable strategy SHA and evidence manifest.
5. Validator Result and Manager decision.
6. Bot Builder Result, including the immutable bot SHA and deployment manifest, when Validator passes.
7. Paper Result and Manager match or mismatch decision.
8. Exact Owner Capital Authorization before any real order.
9. Each owner-defined real-world evidence unit and the resulting continue or stop decision.
10. Final decision, Retrospective, archived experiment, leaderboard change, and accepted lessons.

Manager also commits campaign activation, pause, resume, and completion. Use the commit subject `polyloop(<campaign>/<experiment>): <completed-stage>`; use `campaign` instead of an experiment ID for campaign-only changes. Include the Result, decision, artifact SHAs, functionality-log changes, and evidence references for that boundary. Do not dispatch the next function until the boundary commit exists and the shared worktree is clean. Never amend or rewrite a boundary commit after downstream work has started.
