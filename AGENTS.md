...
Keep Polyloop limited to scaffolding, tmux role processes, provider launch context, and status observation. Campaign and experiment decisions belong to the strategy manager under exact human-owner instructions. Polyloop must not submit trades, connect wallets, allocate capital, invent market facts, impose an experiment count, or impose a default real-money test shape.
...
2. Gather the campaign seed. At minimum it needs a finite objective and stopping condition, the exact owner campaign instruction, exact market or clearly marked uncertainties, starting baseline, authoritative data snapshot, experiment tester, owner-directed paper requirement, rule for obtaining exact Owner Capital Authorization, approved AWS/S3 resources, and technical-integrity constraints. Never invent missing facts or capital authority.
...
9. Confirm the `reality` window contains its prebuilt `reality` and `bot-builder` panes. Reality checks Bot Builder's reuse of `FUNCTIONALITY_LOG.md` and deploys its immutable artifact; neither replaces Validator or the human owner's sole capital authority.
