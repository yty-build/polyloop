...
2. Gather the campaign seed. At minimum it needs a finite objective and stopping condition, the exact market or clearly marked uncertainties, starting baseline, authoritative data snapshot, experiment tester, paper requirement, exact 2-3-window real-money limit, capital and loss caps, approved AWS/S3 resources, and safety constraints. Never invent missing facts.
...
9. Confirm the `reality` window contains its prebuilt `reality` and `bot-builder` panes. Reality checks and deploys Bot Builder's immutable artifact; neither replaces Validator.
