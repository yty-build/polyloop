...
9. Confirm the `bot-reality` window contains its prebuilt `bot-reality` and `bot-integrator` panes. `bot-reality` checks and deploys the integrator's immutable artifact; neither replaces `strat-verifier`.
